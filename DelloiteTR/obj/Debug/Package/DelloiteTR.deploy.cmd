...
set _DeploySetParametersFile=%RootPath%DelloiteTR.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%DelloiteTR.zip" (
echo "%RootPath%DelloiteTR.zip" does not exist. 
...
echo. "%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%DelloiteTR.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension %_MsDeployAdditionalFlags%
"%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%DelloiteTR.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension %_MsDeployAdditionalFlags%
) else (
echo. "%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%DelloiteTR.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -setParamFile:"%RootPath%DelloiteTR.SetParameters.xml" %_MsDeployAdditionalFlags%
"%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%DelloiteTR.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -setParamFile:"%RootPath%DelloiteTR.SetParameters.xml" %_MsDeployAdditionalFlags%
)
goto :eof
...
:usage
echo =========================================================
if not exist "%RootPath%DelloiteTR.deploy-readme.txt" (
...
start notepad "%RootPath%DelloiteTR.deploy-readme.txt"
)
echo =========================================================
goto :eof
...
:CheckParameterFile
echo =========================================================
if  exist "%RootPath%DelloiteTR.SetParameters.xml" (
echo SetParameters from:
echo "%RootPath%DelloiteTR.SetParameters.xml"
echo You can change IIS Application Name, Physical path, connectionString
echo or other deploy parameters in the above file.
) else (
echo SetParamterFiles does not exist in package location.
